...
function generateCustomizeSlug(postProperties, NOTION_CONFIG) {
  if (checkStartWithHttp(postProperties.slug)) return postProperties.slug
  let fullPrefix = ''
  const prefixConfig = siteConfig('POST_URL_PREFIX', BLOG.POST_URL_PREFIX, NOTION_CONFIG)
  const allSlugPatterns = prefixConfig ? prefixConfig.split('/') : []

  allSlugPatterns.forEach((pattern, idx) => {
    if (pattern === '%year%' && postProperties?.publishDay) {
      fullPrefix += new Date(postProperties?.publishDay).getUTCFullYear()
    } else if (pattern === '%month%' && postProperties?.publishDay) {
      fullPrefix += String(new Date(postProperties?.publishDay).getUTCMonth() + 1).padStart(2, '0')
    } else if (pattern === '%day%' && postProperties?.publishDay) {
      fullPrefix += String(new Date(postProperties?.publishDay).getUTCDate()).padStart(2, '0')
    ...
    if (idx !== allSlugPatterns.length - 1) fullPrefix += '/'
  })
